...
Linuxでは他のLinuxにリモートログインできる。これは主に本番やステージング環境にアクセスしたり、バッチサーバにログインしてバッチを実行したりするのに使われる。
...
接続先のサーバによっては秘密鍵の指定が必要な場合がある。その場合は`i`オプションで鍵の場所を指定する。
...
秘密鍵の指定やユーザ名の指定が面倒な場合はホームディレクトリ下の`.ssh`ディレクトリに、configというファイルを用意して設定を記載することで省略できる。
