...
例えば、`/dir1/dir2/dir3/dir4/`というディレクトリの中にある`execfile`というファイルを実行する場合、通常実行コマンドは次になる。
...
大抵のシェルではデフォルトでPATHが設定されている。
PATHを追加したい場合、`bash`を使っている場合は`~/.bash_profile`に`PATH`の設定を行う。
システムによっては既にPATHが設定されているかも知れない。その場合はそこに追記する。
