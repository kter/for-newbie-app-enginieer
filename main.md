...
Linuxでは、すべてのファイルとディレクトリに誰がどういう操作ができるかという権限が設定されている。

権限が足りずアプリケーションが動かないといったトラブルはよくあることなのでしっかり理解して欲しい。

`ls -l .`もしくは`ls -l`でカレントディレクトリのファイルとディレクトリの権限を表示することができる。
出力表示は次のようなものになる。これを例にとって説明を行う。
