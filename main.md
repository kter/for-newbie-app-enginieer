...
WindowsやMacしか使ったことがない方はあまり馴染みがないかも知れないが、環境変数という概念がある。
環境変数とは例えば`HOGE=hage`というような、キー(`HOGE`)と値(`hage`)の組み合わせを指す。慣習的にキーは大文字とすることが多い。
あるLinuxによって、またユーザによって異なる値を格納したい時環境変数を使う。
例えばデフォルトで`HOME=/home/takahashi.t`という環境変数が設定されている。これはユーザによって異なるホームディレクトリの場所を示している。

#### 設定方法

設定方法はいくつかある上にシェル毎に設定方法が違うが、bashの場合は次のコマンドで定義できる。
